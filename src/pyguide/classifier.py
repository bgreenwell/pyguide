...
class GuideTreeClassifier(ClassifierMixin, BaseEstimator):
    ...
    def cost_complexity_pruning_path(self, X, y, sample_weight=None):
        ...
        while not est._root.is_leaf:
            ...
            # Find node with min alpha_eff
            # If multiple nodes have same min alpha, we prune all of them?
            # For simplicity, prune the one with highest depth (closest to leaves) or just min.
            # Sklearn implementation prunes all nodes with the minimum alpha in one step.
            min_alpha = min(c[1] for c in candidates)
            
            # Prune all nodes with this alpha
    ...
    def _collect_pruning_candidates(self, node, n_total, candidates):
        ...
        """
        if node.is_leaf:
            R_leaf = node.impurity * (node.n_samples / n_total)
            return R_leaf, 1
        ...
        R_t = node.impurity * (node.n_samples / n_total)
    ...
    def _calculate_lookahead_gain(self, X, y, split_feat, next_feat):
        ...
        nan_mask = (
            pd.isna(X[:, split_feat]) if not is_cat else pd.isna(X[:, split_feat])
        )
    ...
    def _fit_node(self, X, y, depth):
        ...
        if p > self.significance_threshold and self.interaction_depth > 0:
            ...
            if best_int_p < self.significance_threshold:
                ...
                # Select the best variable from the group to split on.
