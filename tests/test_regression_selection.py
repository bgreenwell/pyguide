...
def test_residual_to_class_logic():
    ...
    # We'll need to expose or test the logic that will be in _fit_node    # For now, let's just test the logic itself if we implement it as a helper
...
def test_regressor_variable_selection_integration():
    ...
    X = np.array([[0, 10], [0, 20], [1, 10], [1, 20]], dtype=float)
